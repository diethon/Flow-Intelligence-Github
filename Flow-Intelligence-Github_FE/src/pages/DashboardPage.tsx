...
const OVERALL_CARD_THEME: Record<
  RiskLevel,
...
const OVERALL_STATUS_MESSAGES: Record<RiskLevel, string> = {
  ...
  medium:
    "Some flow risk rules have exceeded their thresholds. Plan a review with the team to identify areas of review or CI improvement.",
...
export const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    fetchDashboardRepositories()
      ...
      .catch((err) => {
        ...
        setError(
          "Failed to fetch repositories. Please check backend connection.",
        );
        setLoading(false);
      });
  }, []);
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Could not load dashboard analytics. Verify the backend connection and database sync state.",
      );
      setSummary(null);
    } finally {
      setLoading(false);
  ...
  const overallTheme = summary
    ? OVERALL_CARD_THEME[summary.overallRiskLevel]
    : OVERALL_CARD_THEME.good;
    ...
    >
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="h-44 rounded-2xl bg-white animate-pulse border border-slate-200"
              />
            ))}
          </div>
      ...
      {!loading && summary && (
        <div className="space-y-6 animate-fadeIn">
          {selectedRepo?.role && (
            <div className="flex justify-end -mt-5 mb-2">
              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest bg-white border border-slate-200/80 px-2.5 py-1 rounded-lg shadow-sm">
                {selectedRepo.role === "leader"
                  ? "✦ Leader"
                  : selectedRepo.role === "dev"
                    ? "⌘ Developer"
                    : "👁 Viewer"}
              </span>
            </div>
          ...
          <section
            className={`rounded-2xl border p-6 md:p-8 shadow-sm bg-white ${overallTheme.border} ${overallTheme.bg} ${overallTheme.glow}`}
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="space-y-3 flex-1">
                ...
                <h2 className="text-xl md:text-2xl font-extrabold text-slate-900">
                  Pipeline Status is{" "}
                  {summary.overallRiskLevel === "good"
                    ? "Healthy"
                    : summary.overallRiskLevel.toUpperCase() + " RISK"}
                </h2>
                <p
                  className={`text-sm leading-relaxed max-w-2xl ${overallTheme.text}`}
                >
              ...
              <div className="flex flex-col sm:flex-row items-center gap-5 flex-shrink-0 bg-white border border-slate-200 p-5 rounded-2xl shadow-sm">
                <div className="flex items-center gap-4">
                  <div className="text-center">
                    <p className="text-3xl font-extrabold text-rose-500 tabular-nums">
                      {summary.triggeredRuleCount}
                    </p>
                  ...
                  <div className="text-center">
                    <p className="text-3xl font-extrabold text-slate-500 tabular-nums">
                ...
                <PrimaryBtn
                  onClick={() =>
                    navigate(`/repositories/${selectedRepoId}/evidence`)
                  }
                >
                  🔍 View Evidence
                </PrimaryBtn>
              ...
              subtitle={
                windowDays === 0
                  ? startDate && endDate
                    ...
                    : "Normalized metrics calculated over all time"
                ...
                <div className="space-y-4">
                  {summary.bottlenecks.map((btn) => (
                    <BottleneckCard
                      ...
                      card={btn}
                      onDrillDown={() =>
                        navigate(`/repositories/${selectedRepoId}/risk`)
                      }
                    />
                  ))}
                </div>
              ...
              <section className="space-y-6 pt-2 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between gap-3 border-b border-slate-200 pb-2">
                      ...
                      <button
                        onClick={() =>
                          navigate(
                            `/repositories/${selectedRepoId}/predictions`,
                          )
                        }
                    ...
                    summary.recentPredictions.length > 0 ? (
                      summary.recentPredictions.map((pred: any) => (
                        <PredictionCard
                          ...
                          prTitle={
                            pred.pullRequestId?.title ||
                            `Pull Request #${pred.pullRequestId?.number}`
                          }
                          onClick={() =>
                            navigate(
                              `/repositories/${selectedRepoId}/predictions`,
                      ...
                      <p className="text-xs text-slate-500 italic">
                        No recent PR delay predictions.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {summary.recentEvidenceCards &&
                    summary.recentEvidenceCards.length > 0 ? (
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 space-y-4 shadow-sm">
                ...
                <div className="space-y-3 text-xs">
                  <button
                    onClick={() =>
                      navigate(`/repositories/${selectedRepoId}/sync-status`)
                    }
                  ...
                  <button
                    onClick={() =>
                      navigate(`/repositories/${selectedRepoId}/pull-requests`)
                    }
                    className="w-full flex items-center gap-3 p-3 rounded-xl border border-slate-200 hover:bg-slate-50 hover:border-indigo-300 transition-all text-left bg-white shadow-sm"
                  >
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 space-y-4 shadow-sm">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  Metrics are analyzed against 5 team-level flow rules (R1 to
                  R5) configured in the rulebook.
                </p>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-100/50 p-5">
                ...
                <p className="text-[11px] text-slate-500 leading-relaxed">
                  Calculated metrics are restricted to team workflow indicators.
                  HR monitoring, developer rating, and raw source code reading
