...
export function PrivacyPage() {
  ...
  const canViewAutomatedDelivery =
    selectedRepository?.role === "leader" || selectedRepository?.role === "dev";
  ...
  useEffect(() => {
    if (selectedRepoId) {
      if (!canManagePrivacy) {
        setError(
          "You do not have permission to manage privacy settings for this repository.",
        );
        setLoading(false);
      } else {
        loadSettings();
  ...
  const handleTestNotification = async () => {
    ...
    try {
      const res = await briefApi.sendBriefNotification(selectedRepoId, {
        slackWebhookUrl,
      });
      const emailRes = res.data?.notifications?.emailSent
        ? "Email ✅"
        : "Email ⚠️";
      const slackRes = res.data?.notifications?.slackSent
        ? "Slack ✅"
        : "Slack ⚠️";
  ...
  return (
    ...
    >
      ...
      {loading && !settings && (
        <LoadingState message="Loading repository settings..." />
      )}
      ...
      {!loading && !error && settings && (
        <div className="space-y-8 max-w-4xl">
              ...
              <div className="mt-6 space-y-6">
                ...
                <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                  <div>
                    <h4 className="font-semibold text-slate-800">
                      Enable Automatic Weekly Schedule
                    </h4>
                    <p className="text-sm text-slate-500 max-w-lg">
                      Automatically generate AI Brief and send Email & Slack
                      reports on the configured schedule.
                    </p>
                  </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-2">
                    Scheduled reports will be sent automatically to the
                    connected project owner email and Slack channel (if Webhook
                    URL is provided).
                  </p>
                </div>
              </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 md:p-8 shadow-sm">
            <SectionHeading
              title="AI Payload & Privacy Settings"
              subtitle="Control what data is sent to AI models for analysis."
            />
            ...
            <div className="space-y-6 mt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500 max-w-lg">
                    Mask actual developer names with hash-based pseudonyms
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    className="sr-only peer"
                    checked={settings.pseudonymizeContributors}
                    onChange={() => handleToggle("pseudonymizeContributors")}
                  />
                  <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                </label>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-slate-100 pt-5">
                <div>
                  ...
                  <p className="text-sm text-slate-500 max-w-lg">
                    Minimum number of contributors required in a group before
                    generating per-group breakdowns (prevents individual
              ...
              <div className="flex items-center justify-between border-t border-slate-100 pt-5">
                <div>
                  <h4 className="font-semibold text-slate-800">
                    Exclude Raw Comments
                  </h4>
                  <p className="text-sm text-slate-500 max-w-lg">
                    Prevent raw pull request comments and review text from being
                    included in AI payloads.
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    className="sr-only peer"
                    checked={settings.excludeRawComments}
                    onChange={() => handleToggle("excludeRawComments")}
                  />
                  <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                </label>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-slate-100 pt-5">
                <div>
                  <h4 className="font-semibold text-slate-800">
                    Exclude Source Code Diffs
                  </h4>
                  <p className="text-sm text-slate-500 max-w-lg">
                    Ensure no actual source code snippets are sent to external
                    AI providers.
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    className="sr-only peer"
                    checked={settings.excludeRawCode}
                    onChange={() => handleToggle("excludeRawCode")}
                  />
                  <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                </label>
              </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 md:p-8 shadow-sm">
            <SectionHeading
              ...
              subtitle="Manage how your data is retained and shared."
            />
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 mb-6 text-sm text-slate-700">
              ...
              Data generated by this platform is strictly for identifying
              process bottlenecks. It must not be used for individual
              performance evaluation, compensation decisions, or HR punitive
              actions.
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 max-w-lg">
                  Permanently delete or anonymize all records, predictions, and
