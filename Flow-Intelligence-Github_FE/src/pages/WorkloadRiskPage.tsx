      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-6">
        <div>
          <div className="flex items-center gap-2 px-3 py-1 bg-white/60 rounded-full border border-slate-200 mb-2 w-fit">
            ...
            <span className="text-xs font-semibold text-slate-600">
              Workload Risk · {windowDays}d window (UTC)
            </span>
          </div>
        ...
        <div className="sm:border-l sm:border-slate-200 sm:pl-6 flex-1">
          <p className="text-base text-slate-700 leading-relaxed font-medium">
            {a.offHoursEvents} of {a.totalEvents} events ({a.commitCount}{" "}
            commits · {a.reviewCount} reviews) happened outside business hours
            (weekends or nights, UTC), across {a.distinctContributorsOffHours}{" "}
            contributors.
          </p>
        </div>
      </div>
...
function Stat({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-4 py-3">
      <p
        className={`text-2xl font-extrabold tabular-nums ${accent ?? "text-slate-800"}`}
      >
...
function AiAnalysis({
  ...
  ai: NonNullable<WorkloadRiskResult["aiAnalysis"]>;
}) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <AiItemList
            ...
            emptyText="No specific recommendations at this time."
...
> = {
  high: {
    label: "Heavy off-hours load",
    className: "text-rose-600 bg-rose-50 border-rose-100",
  },
  medium: {
    label: "Moderate off-hours load",
...
function ContributorInsights({
  ...
  insights: WorkloadContributorInsight[];
}) {
    ...
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {insights.map((c) => {
          const pct =
            c.totalEvents > 0
              ? Math.round((c.offHoursEvents / c.totalEvents) * 100)
              : 0;
            ...
            >
              <div className="flex items-start justify-between gap-3 mb-3">
                ...
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  <span
                    className={`text-xs font-bold border px-2 py-1 rounded-lg ${badge.className}`}
                  >
                    {badge.label}
                  </span>
    ...
    >
      <summary className="cursor-pointer list-none px-6 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Commits &amp; reviews per contributor, and how many landed at night
            or on weekends (UTC)
          </p>
        </div>
      ...
      <div className="px-6 pb-5 overflow-x-auto">
        ...
        <p className="text-[11px] text-slate-400 mt-3 leading-relaxed">
          <span className="font-semibold text-amber-600">off-hrs</span> =
          commits/reviews made outside business hours ·
          <span className="font-semibold"> 🌙 Night</span> = 20:00–06:00 UTC ·
          <span className="font-semibold"> 📅 Weekend</span> = Saturday/Sunday
          (UTC).
        </p>
      </div>
    </details>
...
export function WorkloadRiskPage({ repositoryId }: WorkloadRiskPageProps) {
  ...
  const [startDate, setStartDate] = useState<string>(
    () => localStorage.getItem("workloadStartDate") || daysAgo(7),
  );
  const [endDate, setEndDate] = useState<string>(
    () => localStorage.getItem("workloadEndDate") || daysAgo(0),
  );
  ...
  const analyzedDays = result
    ? rangeDays(
        result.aggregate.windowStart.slice(0, 10),
        result.aggregate.windowEnd.slice(0, 10),
      )
  ...
  return (
    <PageShell
      ...
      actions={
        <div className="flex flex-wrap items-center justify-start md:justify-end gap-2">
          ...
          <PrimaryBtn
            onClick={runAnalysis}
            disabled={!repositoryId || invalidRange}
    ...
    >
      ...
      {result &&
        result.aggregate.dataStatus === "insufficient_data" &&
        !mutation.isPending && (
          <EmptyState
            icon="🕊️"
      ...
      {/* Content */}
      {result &&
        result.aggregate.dataStatus === "ok" &&
        !mutation.isPending && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-6 gap-3">
              ...
              <Stat
                label="off-hours"
                value={result.aggregate.offHoursEvents}
                accent="text-indigo-600"
              />
              <Stat
                label="at night"
                value={result.aggregate.nightCount}
                icon="🌙"
              />
              <Stat
                label="on weekends"
                value={result.aggregate.weekendCount}
                icon="📅"
              />
              <Stat
                label="contributors"
                value={result.aggregate.distinctContributorsOffHours}
              />
            </div>
            ...
            {result.aiAnalysis && (
              <ContributorInsights
                insights={result.aiAnalysis.contributorInsights}
              />
            )}

            <div className="rounded-xl border border-slate-200 bg-slate-100/30 px-5 py-4">
              <p className="text-xs text-slate-500 leading-relaxed">
                Off-hours are computed from commit &amp; review timestamps in
                UTC (no timezone normalization). Bot commits (e.g. actions-user,
                *[bot]) are credited to the real person in their Co-authored-by
                trailer when present, otherwise excluded — so the signal
                reflects people. The per-contributor breakdown shows real GitHub
                identities; use it to support the team, not to rank or evaluate
                individuals. Names are never sent to the AI provider.
              </p>
            </div>
          </>
        )}
