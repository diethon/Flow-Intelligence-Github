...
export function WeeklyBriefPage() {
  ...
  const loadReportDetails = async () => {
    ...
    if (evidenceResult.status === "fulfilled") {
      ...
      setEvidenceCards(
        (evidenceResult.value.data ?? []).filter((card) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const generated = await briefApi.generateBrief(
        selectedRepoId,
        startIso,
        endIso,
      );
  ...
  const handleExport = (format: "pdf" | "csv") => {
    ...
    const report = {
      brief,
      repositoryName:
        repos.find((repo) => repo._id === selectedRepoId)?.fullName ??
        "Repository",
      evidenceCards,
      metrics,
    };
  ...
  return (
          ...
          <div className="flex flex-wrap items-center justify-start md:justify-end gap-2">
            ...
            <div className="hidden lg:flex flex-shrink-0 items-center gap-2 text-sm text-slate-600 bg-white border border-slate-200 rounded-lg px-2 shadow-sm">
              <input
                ...
                className="w-[8.5rem] bg-transparent border-none focus:ring-0 text-slate-700 py-1.5 outline-none cursor-pointer"
              />
              <span>→</span>
              <input
                type="date"
            ...
            <GhostBtn onClick={loadBriefs} disabled={loading}>
              ↻ Refresh
            </GhostBtn>
            <GhostBtn
            ...
            {canManage && brief?.publicationStatus === "draft" && (
              <GhostBtn onClick={handlePublish} disabled={loading}>
                Publish
              </GhostBtn>
            )}
            {canManage && (
    ...
    >
      ...
      {loading && !brief && (
        <LoadingState message="Fetching your latest AI Brief..." />
      )}
      ...
      {!loading && !error && !brief && repos.length > 0 && (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-slate-200">
          ...
          <h3 className="text-xl font-bold text-slate-800">
            No Weekly Brief Found
          </h3>
          <p className="text-slate-500 mb-6 max-w-md text-center text-sm">
            Generate your first AI-powered Weekly Brief to get a quick summary
            of team workflow risks and predictions.
          </p>
          {canManage ? (
        ...
        <div className="min-w-0 space-y-6">
          ...
          {brief.isFallback && (
            <PartialDataState
              ...
              missingData={["Deterministic rules used for fallback brief."]}
            />
          )}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 md:p-8 shadow-sm">
            ...
            <p className="break-words text-slate-700 leading-relaxed">
              {brief.summary}
            </p>
          </div>
          ...
          {brief.items.some((i) => i.type === "trend_comparison") && (
            <div className="rounded-2xl border border-slate-200 bg-white p-6 md:p-8 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {brief.items
                  .filter((i) => i.type === "trend_comparison")
                  .map((item, idx) => (
                    <div
                      key={idx}
                      className="min-w-0 bg-blue-50 border border-blue-100 rounded-xl p-4"
                    >
                      <h4 className="break-words font-semibold text-blue-900 mb-1">
                        {item.title}
                      </h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="min-w-0 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <ul className="space-y-3">
                {brief.items
                  .filter((i) => i.type === "risk_summary")
                  .map((item, idx) => (
                    <li key={idx} className="border-l-4 border-amber-400 pl-3">
                      <h4 className="break-words font-semibold text-slate-800">
                        {item.title}
                      </h4>
                ...
                {brief.items.filter((i) => i.type === "risk_summary").length ===
                  0 && (
              ...
              <ul className="space-y-3">
                {brief.items
                  ...
                  .map((item, idx) => (
                    <li
                      key={idx}
                      className="border-l-4 border-emerald-400 pl-3"
                    >
                      <h4 className="break-words font-semibold text-slate-800">
                        {item.title}
                      </h4>
                ...
                {brief.items.filter((i) => i.type === "recommendation")
                  .length === 0 && (
                  <p className="text-sm text-slate-500 italic">
